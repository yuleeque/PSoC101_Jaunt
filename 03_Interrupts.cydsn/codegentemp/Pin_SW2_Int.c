...
CY_ISR(Pin_SW2_Int_Interrupt)
{
    ...
    /* PSoC3 ES1, ES2 RTC ISR PATCH  */ 
        ...
        #if((CYDEV_CHIP_REVISION_USED <= CYDEV_CHIP_REVISION_3A_ES2) && (Pin_SW2_Int__ES2_PATCH ))      
            Pin_SW2_Int_ISR_PATCH();
...
{
    CY_SET_REG16(Pin_SW2_Int_INTC_VECTOR, (uint16) address);
}
...
{
    return (cyisraddress) CY_GET_REG16(Pin_SW2_Int_INTC_VECTOR);
}
...
uint8 Pin_SW2_Int_GetState(void) 
{
    ...
    return ((*Pin_SW2_Int_INTC_SET_EN & (uint8)Pin_SW2_Int__INTC_MASK) != 0u) ? 1u:0u;
}
